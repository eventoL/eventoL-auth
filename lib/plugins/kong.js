...
function create(context, application, user) {
    return kongUtils.addConsumer(context.kongUrl, user._id, context.kongApiKeyPath, context.kongApiKey)
        .then((res) => {
            logger.info('Consumer created: ', res);
            return kongUtils.addJWTCredentials(context.kongUrl, user._id,
...
function remove(context, application, user) {
    return kongUtils.deleteConsumer(context.kongUrl, user._id, context.kongApiKeyPath, context.kongApiKey)
        .then(() => user);
}
