...
function create(context, application, user) {
    return kongUtils.addConsumer(context.kongUrl, user.nickname, user._id)
        .then((res) => {
...
function remove(context, application, user) {
    return kongUtils.deleteConsumer(context.kongUrl, user.username, context.kongApiKeyPath, context.kongApiKey)
        .then(() => user);
}

function issueToken(profile, clientSecret, jwtOptions) {
    return jsonwebtoken.sign(profile, clientSecret + profile.id.toString(), jwtOptions);
}
