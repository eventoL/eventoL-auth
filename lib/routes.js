...
const buildAuthManager = require('./authManager');
const bodyParser = require('body-parser');

function initializeJwtPassports(passport, authManager, config) {
...
function buildAuthRouter(config) {
    ...
    const authManager = buildAuthManager(config);

    initializeJwtPassports(passport, authManager, {
        jwt: config.jwt
    });
    ...
    authManager.availableProviders.forEach((provider) => {
        authRouter.post(`/auth/${provider}/token`, (req, res) => {
            authManager
                ...
                .catch((authError) => res.status(401).json({
    ...
    authRouter.post('/auth/revoke/token', passport.authenticate('jwtNoExpiration', {
        session: false
    }), (req, res) => {
        authManager
