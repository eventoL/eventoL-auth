...
AppSchema.methods.issueToken = function issueToken(profile) {
    ...
    Reflect.deleteProperty(profile, '_id');
    Reflect.deleteProperty(profile, '__v');
    ...
    if (this.hasIssueTokenPlugin()) {
        return this.getIssueTokenPlugin().issueToken(profile, this.clientSecret, jwtOptions);
    }

    return jsonwebtoken.sign({
        profile
    }, this.clientSecret, jwtOptions);
};
        ...
        .then((user) => {
            if (user === null || typeof user === 'undefined') {
                ...
                validate(!oauthConnection.signupEnabled,
                    `The ${provider} is not available for registration in this app`, 404);
...
AppSchema.methods.getIssueTokenPlugin = function getIssueTokenPlugin() {
    return this.plugins.filter((plugin) => plugin.issueTokenEnabled)[0];
};

AppSchema.methods.applyPlugins = function applyPlugins(user, operation) {
    return this.plugins.reduce((user, pluginDoc) => {
        const plugin = plugins[pluginDoc.name];
        return Reflect.apply(plugin[operation], plugin, [pluginDoc.context, this, user]);
    }, user);
};
