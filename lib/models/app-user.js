    ...
    return this
        ...
        .then((user) => {
            if (user === null || typeof user === 'undefined') {
                const oauthConnection = application.oauthConnections
                    .filter((oauthConnection) => oauthConnection.connection === connection)[0];

                if (!oauthConnection.signupEnabled) {
                    const error = new Error(`The ${provider} profile haven't been connected to an existing user.`);
                    error.status = 404;
                    throw error;
                }

                if (application.whiteList.length > 0 && !application.whiteList.includes(email)) {
                    const error = new Error(`The email ${email} isn't part off the application's white list.`);
                    error.status = 403;
                    throw error;
                }
