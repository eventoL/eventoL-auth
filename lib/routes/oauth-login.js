...
function oauthLogin(req, res, next) {
    ...
    return App.validate(appId, provider)
        .then((application) => {
            ...
            const callbackUrl     = `${process.env.EVENTOL_AUTH_API_HOST}/api/app/${appId}/${provider}/callback`;
...
function linkUserLogin(req, res, next) {
    ...
    return Promise.all([JWTUtils.getJWT(req), App.validate(appId, connection)])
        .then((userJWT, application) => {
            return application
                .verifyJWT(userJWT)
                .then(() => application);
        })
        .then((application) => {
            ...
            const oauthAdapter    = oauthAdapters[oauthConnection.type];
            const callbackUrl     = `${process.env.EVENTOL_AUTH_API_HOST}/api/app/${appId}/${connection}/link/callback`;
